...
import com.debdroid.apiwithjdo.dao.employeeendpoint.model.CollectionResponseEmployee;
...
public class ListEmployee extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.d("DEB", "About to call the AsyncTask");
	...
	private class EmployeeListAsyncTask extends
			AsyncTask<Void, Void, CollectionResponseEmployee> {
		private ProgressDialog pd;
		Context context;

		public EmployeeListAsyncTask(Context ctx) {
			context = ctx;
		}
		...
		protected CollectionResponseEmployee doInBackground(Void... unused) {
			CollectionResponseEmployee employees = null;
			try {
				Employeeendpoint.Builder builder = new Employeeendpoint.Builder(
						AndroidHttp.newCompatibleTransport(),
						new GsonFactory(), null);
				Employeeendpoint service = builder.build();
				employees = service.listEmployee().execute();
			} catch (Exception e) {
		...
		protected void onPostExecute(CollectionResponseEmployee employees) {
			pd.dismiss();
			if (employees != null) {
				...
				Toast.makeText(getBaseContext(),
						"Not able to get the lis from server!",
